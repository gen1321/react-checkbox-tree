...
gulp.task(
    "test-script",
    gulp.series("test-script-format", "test-script-mocha")
);

gulp.task(
    "build-script",
    gulp.series("test-script", () =>
        gulp
            .src(["./src/index.js"])
...
gulp.task(
    "compare-css-output",
    gulp.series(gulp.parallel("build-style", "build-style-less"), () =>
        gulp
            .src("./gulpfile.js")
            .pipe(
                exec(
                    "cmp .css-compare/less/react-checkbox-tree.css .css-compare/scss/react-checkbox-tree.css"
                )
            )
            .pipe(exec.reporter())
    )
...
gulp.task("build-examples-style-prod", () => buildExamplesStyle(true));
...
gulp.task(
    ...
    gulp.series(
        ...
        () => {
            ...
            gulp.watch(["./src/js/**/*.js", "./examples/src/**/*.js"]).on(
                "change",
                gulp.series("build-examples-script")
            );
            gulp.watch(["./src/scss/**/*.scss", "./examples/src/**/*.scss"]).on(
                "change",
                gulp.series("build-examples-style")
            );
            gulp.watch(["./examples/src/**/*.html"]).on(
                "change",
                gulp.series("build-examples-html", browserSync.reload)
            );
        }
    )
);
...
gulp.task(
    "build-gh-pages",
    gulp.parallel(
        "build-examples-style-prod",
        "build-examples-script-prod",
        "build-examples-html"
    )
);
